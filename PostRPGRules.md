...
* **Адреналин (ДЕЙСТВИЕ)(ОДНОРАЗОВЫЙ):** восстанови себе `1 HP` (не выше максимума).
...
* **Схватить пульс (ДЕЙСТВИЕ)(ОДНОРАЗОВЫЙ):** подлечи **союзника рядом** на **`2 HP`** (не выше максимума).
...
* **Прикрой! (РЕАКЦИЯ):** когда по **союзнику рядом** стреляют и **попадают**, дай ему **`+2 AC` против этого попадания** (может превратить его в промах).

**Стоимость 3**

* **Предугадывание (РЕАКЦИЯ)(ОДНОРАЗОВЫЙ):** враг перебрасывает свой `d20` и берёт худший результат.
* **Железная воля (РЕАКЦИЯ)(ОДНОРАЗОВЫЙ):** по тебе попали - снизь урон на `−1d6` (мин. 0).
* **Выдернуть с края (ДЕЙСТВИЕ) (ОДНОРАЗОВЫЙ):** **союзник рядом** в состоянии **«Вырублен»** **немедленно приходит в
  себя с `1 HP`**, снимает состояние. В свой **следующий ход** он может только **отползти** или **встать**.
...
**REQ:** `INT 4+`
**Эффект:** **+1** к броскам атаки **тяжёлым вооружением**; игнорируешь **натуральную `1`** при атаке тяжёлым: считай
бросок как **`2`** (без неприятности).
