...
* `19-20` - успех: очнулся с `1 HP`; в следующий свой ход можешь действовать как обычно.
...
Можно съесть **сколько угодно по очереди**, кидая кубик **за каждый отдельно**.
...
*Костяшки - как арматура, челюсти - как мишени.*  
...
*Ты не молишься богам Пустоши - ты их измеряешь.*  
**Требуется:** `INT 3+`  
...
*Нож тупой, игла кривая - но руки твёрдые.*  
...
**Эффект:** `+1 AC` `-2 INIT` (постоянно); действие "укрепиться" даёт `+2 AC` вместо `+1 AC`.

#### ♟️ Шахматист Руин

*Бой для тебя - позиционная задачка: шаг туда, щелчок здесь, и кто-то падает.*  
**Требуется:** `INT 4+`  
**Эффект:** `+1 SMP` (постоянно) и `+3 INIT`.

#### ♟️ Острый Ум

*TODO*  
**Требуется:** `INT 5+`  
**Эффект:** `+1 SMP` (постоянно) и `+3 урона` при любом критическом попадании.
...
получаешь `3 Сухпаек` в инвентарь;
